...
var db = mongojs('mongodb+srv://Chandu:Messi1234@cluster0.pttedks.mongodb.net/Cspace?retryWrites=true&w=majority',['mentor','mentee','messages','mymentors']);
const saltRounds = 10;
            ...
            db.mentee.find({RegistrationNumber: RegdNo},(err,response)=>{
                if(response.length == 0){
                    db.mentee.find({Telegram: telegram},(err,resp)=>{
                        if(resp.length == 0){
                            db.mentee.insert({FirstName:FirstName,LastName:LastName,Email:Email,Password:hash,RegistrationNumber:RegdNo,Mobileno:Mobileno,Telegram:telegram,isAvatharImageSet:false,AvatharImage : "",users:[]},(error,res1)=>{
